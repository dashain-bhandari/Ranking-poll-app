...
const Voting = () => {
    ...
    const onStartVoting = () => {
        ...
        if (poll.nominations.length < poll.votesPerVoter) {
            ...
            toast.error("There are no other users!!!")
        }
        else {
            ...
            <div className="w-full h-full flex-col justify-center items-center  mb-2  mt-12">
                {
                    !poll.votingStarted && user.userId == poll.admin && !poll.votingEnded && <div className="  flex justify-center items-center  w-full h-full " >
                        <button className="bg-[#4B0082]   hover:bg-[#4B3F72] rounded w-fit p-4 text-white" onClick={onStartVoting}>
                            Start poll
                        </button>
                    </div>
                }
                ...
                }
                {
                    poll?.votingStarted && !poll.votingEnded && (!poll.votes || !poll.votes.find((i) => i.user == user.userId)) && (<>
                        <div className='flex flex-col gap-4 w-full p-8 self-start'>
                            ...
                            {
                                poll && poll?.nominations?.length > 0 && (<button className={`bg-[#4B0082]   hover:bg-[#4B3F72] px-4 py-2 rounded w-fit text-white flex ${votes.length < poll.votesPerVoter &&
...

                {
                    poll && poll.votingStarted && !poll.votingEnded && poll.admin == user.userId && (<>
                        ...
                        <div className="flex w-full flex-col justify-center items-center">
                            ...
                            <div className=" bg-[#4B0082]  hover:bg-[#4B3F72]cursor-pointer rounded px-4 py-2 text-white w-fit " onClick={endPoll}>End poll</div>
...

                {
                    poll && poll.votingEnded && vote && (<div className="flex flex-col  w-full  p-4 gap-4  "  >
                        <div className="self-end">
                            <button className="px-4 py-2 bg-[#8980F5] rounded  self-end hover:bg-violet-600 text-white " onClick={handlePrint}>
                                Download
                            </button>
                        </div>
                        ...
                        <div className="w-full min-w-[400px] flex justify-center w-full md:w-2/3" id="divToPrint" ref={pdf}>  {poll && weights && <Bar className=""
                            data={chart}
                            options={{
                        ...
                        <table className="w-full text-sm text-left rtl:text-right text-gray-500 shadow-md  ">
                            <thead className="w-full text-xs text-gray-900 uppercase bg-violet-300  "
                            ><tr className="">
